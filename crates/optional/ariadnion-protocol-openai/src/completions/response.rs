// crates/optional/ariadnion-protocol-openai/src/completions/response.rs - Legacy Completions JSON and error projection.
//
...
//
//! Bounded legacy Completions JSON and redacted error projection.
...
pub(crate) struct OpenAiCompletionsProjection {
    model: Box<str>,
    include_usage: bool,
    created: u64,
}

impl OpenAiCompletionsProjection {
    pub(crate) fn new(model: Box<str>, include_usage: bool, created: u64) -> Self {
        Self {
            model,
            include_usage,
            created,
...
impl Debug for OpenAiCompletionsProjection {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("OpenAiCompletionsProjection")
            .field("include_usage", &self.include_usage)
            .finish_non_exhaustive()
    }
}

impl HttpProtocolProjection for OpenAiCompletionsProjection {
...
fn complete_response(
    ...
    let body = CompletionBody {
        ...
        usage: UsageBody::zero(),
    };
    json_response(StatusCode::OK, &body)
}

fn validate_response(response: &TextServiceResponse) -> Result<(), ProtocolFailure> {
...
pub(crate) fn completion_id(identity: &HttpRequestIdentity) -> String {
    format!("cmpl-{}", identity.request_id().as_str())
}

pub(crate) fn finish_reason(reason: FinishReason) -> Result<&'static str, ProtocolFailure> {
    match reason {
...
const fn domain_failure_profile(code: ApiDomainErrorCode) -> ErrorProfile {
    match code {
        ...
        _ => domain_runtime_failure_profile(code),
    }
}

const fn domain_runtime_failure_profile(code: ApiDomainErrorCode) -> ErrorProfile {
    match code {
...
#[derive(Serialize)]
pub(crate) struct CompletionChoice<'a> {
    text: &'a str,
    index: u8,
    logprobs: Option<()>,
