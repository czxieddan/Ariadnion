// crates/optional/ariadnion-storage-rnmdb/src/invitation_repository/evidence.rs - Rust source for Ariadnion.
//
...
//
//! Deterministic audit and outbox evidence for invitation issuance.
...
use ariadnion_invitation::{Invitation, InvitationEventKind, InvitationVersion};
...
use crate::invitation_repository::decode::event::PersistedInvitationEvent;
use crate::outbox::enqueue_message;

const SUBJECT_DOMAIN: &[u8] = b"ariadnion.invitation.audit-subject.v1\0";
const IDENTITY_DOMAIN: &[u8] = b"ariadnion.invitation.transition.identity.v1\0";
const SNAPSHOT_DOMAIN: &[u8] = b"ariadnion.invitation.snapshot.v1\0";
const PAYLOAD_DOMAIN: &[u8] = b"ariadnion.invitation.transition.payload.v1\0";
const AUDIT_ID_DOMAIN: &[u8] = b"ariadnion.invitation.audit-event-id.v1\0";
const OUTBOX_ID_DOMAIN: &[u8] = b"ariadnion.invitation.outbox-event-id.v1\0";
const OUTBOX_KEY_DOMAIN: &[u8] = b"ariadnion.invitation.outbox-idempotency.v1\0";
const OUTBOX_TOPIC: &str = "identity.invitation.lifecycle.v1";
...
pub(super) fn reconcile_transition_evidence(
    ...
    let record = EvidenceRecord::from_request(request);
    reconcile_record(session, record, key, request.context)
}
...
pub(super) fn verify_later_transition_evidence(
    ...
    key: &AuditSubjectKeyMaterial,
    durable: &Invitation,
    later: &PersistedInvitationEvent,
) -> Result<(), StorageError> {
    let record = EvidenceRecord::from_persisted(origin, durable, later);
    reconcile_record(session, record, key, origin.context).map(|_| ())
}
...
struct EvidenceRecord {
    ...
    user_id: Option<ariadnion_user_domain::UserId>,
}
...
impl EvidenceRecord {
    ...
    fn from_persisted(
        ...
        Self {
            tenant: origin.tenant_id.clone(),
            organization: origin.organization_id.clone(),
            invitation: durable.clone(),
            expected_previous_version: InvitationVersion::initial(),
            actor: event.actor().clone(),
            occurred_at: event.occurred_at(),
            kind: event.kind(),
...
impl TransitionEvidence {
    ...
    fn audit_event(
        ...
        let content = AuditEventContent::new(
            ...
            AuditSubject::from_digest(AuditSubjectKind::Invitation, self.subject),
...
fn canonical_identity(record: &EvidenceRecord) -> Result<Zeroizing<Vec<u8>>, StorageError> {
    ...
    push_field(&mut output, &snapshot_digest(&record.invitation)?)?;
    Ok(output)
}
...
fn push_identity_boundary(
    ...
    push_text(output, record.organization.as_str())?;
    push_text(output, record.invitation.id().as_str())
}
...
fn push_identity_transition(
    ...
    push_u64(output, record.invitation.version().get())?;
    ...
    push_optional_text(output, record.user_id.as_ref().map(|user| user.as_str()))
}

fn snapshot_digest(invitation: &Invitation) -> Result<[u8; 32], StorageError> {
    let mut snapshot = Zeroizing::new(SNAPSHOT_DOMAIN.to_vec());
    push_snapshot_binding(&mut snapshot, invitation)?;
    push_snapshot_proofs(&mut snapshot, invitation)?;
    push_snapshot_lifecycle(&mut snapshot, invitation)?;
    Ok(Sha256::digest(&snapshot).into())
}

fn push_snapshot_binding(
    snapshot: &mut Vec<u8>,
    invitation: &ariadnion_invitation::Invitation,
) -> Result<(), StorageError> {
    push_text(snapshot, invitation.tenant_id().as_str())?;
    push_text(snapshot, invitation.organization_id().as_str())?;
    push_text(snapshot, invitation.id().as_str())?;
    push_text(snapshot, invitation.issuer().as_str())
}

fn push_snapshot_proofs(
    snapshot: &mut Vec<u8>,
    invitation: &ariadnion_invitation::Invitation,
) -> Result<(), StorageError> {
    push_field(snapshot, &invitation.subject_digest().bytes())?;
    push_field(snapshot, &invitation.token_digest().bytes())
}
...
fn push_snapshot_lifecycle(
    ...
    push_i64(snapshot, invitation.issued_at().unix_seconds())?;
    push_i64(snapshot, invitation.expires_at().unix_seconds())?;
    push_u64(snapshot, invitation.version().get())?;
    push_text(snapshot, super::sql::state_label(invitation.state()))?;
    push_optional_text(snapshot, invitation.consumed_by().map(|user| user.as_str()))
}
...
fn subject_digest(
    ...
    push_text(&mut material, record.organization.as_str())?;
    push_text(&mut material, record.invitation.id().as_str())?;
...
fn derive_audit_id(canonical: &[u8]) -> Result<AuditEventId, StorageError> {
    let value = derived_id(AUDIT_ID_DOMAIN, "invitation-audit-v1-", canonical)?;
    AuditEventId::parse(&value).map_err(|_| integrity_failure())
}

fn derive_outbox_id(canonical: &[u8]) -> Result<OutboxEventId, StorageError> {
    let value = derived_id(OUTBOX_ID_DOMAIN, "invitation-outbox-v1-", canonical)?;
...
fn derive_outbox_key(canonical: &[u8]) -> Result<OutboxIdempotencyKey, StorageError> {
    let value = derived_id(OUTBOX_KEY_DOMAIN, "invitation-transition-v1-", canonical)?;
...
const fn audit_event_kind(kind: InvitationEventKind) -> AuditEventKind {
    match kind {
        InvitationEventKind::Issued => AuditEventKind::Issued,
        InvitationEventKind::Consumed => AuditEventKind::Consumed,
        InvitationEventKind::Revoked => AuditEventKind::Revoked,
        InvitationEventKind::Expired => AuditEventKind::Expired,
    }
}

const fn audit_reason(kind: InvitationEventKind) -> &'static str {
    match kind {
        InvitationEventKind::Issued => "INVITATION_ISSUED",
        InvitationEventKind::Consumed => "INVITATION_CONSUMED",
        InvitationEventKind::Revoked => "INVITATION_REVOKED",
        InvitationEventKind::Expired => "INVITATION_EXPIRED",
    }
}
