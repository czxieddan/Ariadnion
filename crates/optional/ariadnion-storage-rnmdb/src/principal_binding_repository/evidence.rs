// crates/optional/ariadnion-storage-rnmdb/src/principal_binding_repository/evidence.rs - Rust source for Ariadnion.
//
...
//
//! Deterministic audit and outbox evidence for principal-binding transitions.
...
use ariadnion_principal_binding::{
    PrincipalBindingEventKind, PrincipalBindingSnapshot, PrincipalBindingState,
    PrincipalBindingTransition, SubjectCommitment,
};
...
const SUBJECT_DOMAIN: &[u8] = b"ariadnion.principal-binding.audit-subject.v1\0";
const IDENTITY_DOMAIN: &[u8] = b"ariadnion.principal-binding.transition.identity.v1\0";
const PAYLOAD_DOMAIN: &[u8] = b"ariadnion.principal-binding.transition.payload.v1\0";
const AUDIT_ID_DOMAIN: &[u8] = b"ariadnion.principal-binding.audit-event-id.v1\0";
const OUTBOX_ID_DOMAIN: &[u8] = b"ariadnion.principal-binding.outbox-event-id.v1\0";
const OUTBOX_KEY_DOMAIN: &[u8] = b"ariadnion.principal-binding.outbox-idempotency.v1\0";
const OUTBOX_TOPIC: &str = "identity.principal-binding.lifecycle.v1";
...
impl EvidenceIdentity {
    fn new(
        ...
        let canonical = canonical_identity(transition)?;
        ...
        Ok(Self {
            ...
            subject: subject_digest(
                ...
                transition.binding().subject_commitment(),
                key,
            )?,
...
impl TransitionEvidence {
    ...
    fn audit_event_at(
        ...
        let content = AuditEventContent::new(
            ...
            AuditSubject::from_digest(AuditSubjectKind::Administration, self.subject),
...
fn canonical_identity(
    ...
    push_identity_boundary(&mut output, transition)?;
    push_transition_snapshots(&mut output, transition)?;
    push_transition_event(&mut output, transition)?;
    Ok(output)
}
...
fn push_identity_boundary(
    ...
    push_text(output, transition.principal_id().as_str())
}

fn push_transition_snapshots(
    output: &mut Vec<u8>,
    transition: &PrincipalBindingTransition,
) -> Result<(), StorageError> {
    push_optional_u64(
    ...
    push_optional_snapshot(output, transition.previous_snapshot())?;
    push_snapshot(output, &transition.new_snapshot())
}
...
fn push_transition_event(
    ...
    push_field(output, event.subject_commitment().as_bytes())
}

fn push_optional_snapshot(
    output: &mut Vec<u8>,
    snapshot: Option<&PrincipalBindingSnapshot>,
) -> Result<(), StorageError> {
    match snapshot {
        Some(snapshot) => {
            push_field(output, &[1])?;
            push_snapshot(output, snapshot)
        }
...
fn push_snapshot(
    output: &mut Vec<u8>,
    snapshot: &PrincipalBindingSnapshot,
) -> Result<(), StorageError> {
    push_snapshot_identity(output, snapshot)?;
    push_snapshot_lifecycle(output, snapshot)
}
...
fn push_snapshot_identity(
    ...
    push_field(output, snapshot.subject_commitment().as_bytes())?;
...
fn push_snapshot_lifecycle(
    ...
    push_i64(output, snapshot.provisioned_at().unix_seconds())?;
    push_optional_i64(
        output,
    ...
    push_optional_i64(
        output,
        snapshot.erased_at().map(|value| value.unix_seconds()),
    )?;
    push_field(output, &[u8::from(snapshot.identity().is_some())])
}
...
fn subject_digest(
    ...
    push_text(&mut material, principal.as_str())?;
    push_field(&mut material, commitment.as_bytes())?;
...
fn derive_audit_id(canonical: &[u8]) -> Result<AuditEventId, StorageError> {
    let value = derived_id(AUDIT_ID_DOMAIN, "principal-binding-audit-v1-", canonical)?;
    AuditEventId::parse(&value).map_err(|_| integrity_failure())
}

fn derive_outbox_id(canonical: &[u8]) -> Result<OutboxEventId, StorageError> {
    let value = derived_id(OUTBOX_ID_DOMAIN, "principal-binding-outbox-v1-", canonical)?;
...
fn derive_outbox_key(canonical: &[u8]) -> Result<OutboxIdempotencyKey, StorageError> {
    let value = derived_id(
        OUTBOX_KEY_DOMAIN,
        "principal-binding-transition-v1-",
        canonical,
    )?;
...
fn push_optional_u64(output: &mut Vec<u8>, value: Option<u64>) -> Result<(), StorageError> {
    match value {
        Some(value) => {
            push_field(output, &[1])?;
...
const fn state_label(state: PrincipalBindingState) -> &'static str {
    match state {
...
const fn audit_event_kind(kind: PrincipalBindingEventKind) -> AuditEventKind {
    match kind {
        PrincipalBindingEventKind::Provisioned => AuditEventKind::Issued,
        PrincipalBindingEventKind::Revoked => AuditEventKind::Revoked,
        PrincipalBindingEventKind::Erased => AuditEventKind::Administered,
    }
}

const fn audit_reason(kind: PrincipalBindingEventKind) -> &'static str {
    match kind {
        PrincipalBindingEventKind::Provisioned => "PRINCIPAL_BINDING_PROVISIONED",
        PrincipalBindingEventKind::Revoked => "PRINCIPAL_BINDING_REVOKED",
        PrincipalBindingEventKind::Erased => "PRINCIPAL_BINDING_ERASED",
    }
}
